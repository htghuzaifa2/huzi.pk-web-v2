...
First, let's understand the betrayal. `ForceCompositionPipeline` is an X11-specific fix. It commands the NVIDIA driver to take full control of the compositing process within the X Window System, ensuring frames are perfectly synchronized before being sent to the display.

Wayland abolishes this entire model. There is no "X Server Display Configuration" because there is no X Server in the traditional sense. The compositor (like KWin or Sway) is the display server. When you run a Wayland session, the NVIDIA driver's X11 control panel is talking to a ghost. The checkbox you check has no effect on your actual, running Wayland desktop.

The tearing you see on Wayland is a different beast. It's often a symptom of missing **explicit synchronization** ("explicit sync") between the application, the compositor, and the NVIDIA driver. Without it, frames are delivered out of sync with your monitor's refresh cycle. This is the core issue that needed to be solved.
...
Your entire quest rests on this foundation. The explicit sync support that fixes Wayland tearing for NVIDIA was merged into the **NVIDIA 555 driver series** and requires corresponding patches in your desktop environment.
*   **Check Your Driver:** Ensure you are using NVIDIA driver 555.xx.xx or newer. Older drivers (like 470, 515, or 535) fundamentally lack the necessary support.
*   **Verify Your Desktop Environment:** This fix is most mature in **KDE Plasma 6** (with the explicit sync patch) and **GNOME 46+**. If you are on an older desktop or a different compositor, you may still experience issues.

### Step 2: Diagnose the "Hybrid Graphics" Trap
A huge source of persistent tearing, especially on laptops, is misconfigured hybrid graphics (e.g., an NVIDIA GPU paired with an Intel or AMD integrated GPU). The system might be rendering frames on your powerful NVIDIA card but displaying them through the slower integrated GPU, causing a bottleneck and tears.

**How to diagnose?** Run `glxinfo -B` in a terminal. Look at the "OpenGL renderer" line. If it lists your Intel or AMD GPU instead of your NVIDIA GeForce, this is your problem.

**The solution** is to ensure your display is physically connected to and managed by the NVIDIA GPU. This can often be configured in your BIOS/UEFI (look for a "Discrete Graphics Only" or "dGPU" mode). On some KDE setups, you can use the command `kscreen-doctor` to reconfigure outputs.

### Step 3: Tame the XWayland Tunnel
Many applications, including most games and Steam, still run through XWayland (a compatibility layer that translates X11 calls to Wayland). Explicit sync must work through this tunnel too.
...
*   **Test a Native Wayland App:** Open a terminal like konsole (ensure it's running natively on Wayland) and some native app like plasma-systemmonitor. Do they tear? If only XWayland apps (like your games) tear, the issue is more isolated.
...
2.  **The Compositor Swap Test:** As discovered by one user, tearing that is persistent in KDE (KWin) may vanish on a different Wayland compositor like Sway. This points to a compositor-specific bug or misconfiguration. Testing Sway briefly can confirm if this is your escape route.
...
flowchart TD
    ...
    subgraph L [Advanced Workarounds]
        ...
        L2[Test different compositor e.g., Sway]
