...
## Here is your immediate action plan to reclaim your audio device:
...
The `-v` flag gives a verbose output. It will list the Process ID (PID) and the user of any process using that file. Once you have the PID, you can investigate it with `ps aux | grep <PID>` and then stop the application gracefully or use `kill <PID>`.
...
```bash
pw-cli ls Link | grep -A5 -B5 "state: playing"
```
...
flowchart TD
    ...
    E -- Yes --> F[Investigate Process:<br><code>ps aux | grep &lt;PID&gt;</code>];
