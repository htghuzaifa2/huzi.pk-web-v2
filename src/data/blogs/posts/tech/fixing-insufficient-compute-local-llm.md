...
Let’s start with the immediate interventions—the commands and config changes you can make in the next ten minutes to get your cluster breathing again.
...
*   **For GPU Memory:** Run `nvidia-smi` in a terminal. Watch it in real-time as you launch your models. Which process is consuming the most VRAM? Is memory fragmented?
...
*   **The Quick Command:** If using ollama, pull the quantized variant: `ollama pull llama2:7b-chat-q4_0`. The `q4_0` is the quantization tag.
...
Think of your cluster as a busy kitchen serving many different types of meals (models). You need a head chef (orchestrator), mise en place (optimized models), and clear tickets (efficient requests).

| Strategy | Core Principle | Tool/Example | Expected Gain |
| :--- | :--- | :--- | :--- |
| **Aggressive Quantization** | Trade negligible accuracy for massive memory savings. | Using GGUF (llama.cpp) or GPTQ models. | 60-70% memory reduction (16-bit → 4-bit). |
...
*   **CPU vs. GPU:** For CPU-only inference, your best friend is **llama.cpp** with GGUF models. It’s surprisingly capable, especially with fast RAM and modern CPUs. For GPU inference, VRAM is king. Memory bandwidth is your secret god.
...
Your software is the nervous system that manages the hardware body.
...
*   **For High-Throughput API Servers:** **vLLM** or **TGI**. These are production-grade systems. They use PagedAttention and continuous batching to serve hundreds of requests per second with minimal memory waste. This is how you turn a cluster into a real service.
