...
Before we dive into the machinery, try these steps. They resolve the majority of sudden Bluetooth audio dropouts.
...
2.  Close Zoom/any conferencing app completely.
3.  Reconnect your headset. Wait for the system confirmation sound.
4.  Now, open Zoom. Join the meeting.
This simple sequence forces a fresh negotiation cycle and often solves the issue instantly.
...
```
This restarts the core audio services without needing a reboot.
...
*   **A2DP (Advanced Audio Distribution Profile):** For high-quality, stereo music playback. It's a one-way street for sound to you. The microphone is disabled.
*   **HSP/HFP (Hands-Free Profile):** For calls. This is two-way, lower-bandwidth, mono sound. It enables both your microphone and your earpiece.
...
Even within these profiles, there are different audio codecs (SBC, AAC, aptX, LDAC for A2DP; mSBC, CVSD for HFP). Your system, your headset, and Zoom (via PulseAudio protocol) have preferences. If Zoom aggressively requests a specific codec or sample rate that your current PipeWire->Bluetooth setup can't handle seamlessly, the audio graph can collapse, causing the dropout.

### The Buffer Mystery
PipeWire uses a buffer to manage audio data flow. If the buffer size is too small for the Bluetooth link (which can be laggy and variable), underruns happen—the audio pipeline empties, causing dropouts. Zoom starting can change the demanded latency, triggering this.
...
2.  Create a custom rule for it. Create or edit a PipeWire configuration override:
    ...
    ```
3.  Add the following to increase buffer size (adjust the `node.name` and `audio.rate`):
    ```text
    context.properties = {
    ...
    context.objects = [
        {
            ...
            args = {
                ...
                # Increase min.quantum for larger buffer (more stability, slightly more latency)
                api.alsa.period-size   = 1024
                api.alsa.headroom      = 8192
                stream.props = {
                    # Key line: Increases minimum buffer time to 120ms (default is often 60ms)
                    pipewire.min.quantum = 5120/48000 # ~120ms
                }
            }
        }
    ]
